...
Search(searchText: string) {
  ...
  const searchRecursive = (menus: Menu[]) => {
    for (const item of menus) {

      // 🔐 1. Filtrar por permisos
      if (!this.hasPermission(item)) continue;

      // 🌍 2. Traducir el título REAL
      if (!item.title) continue;
      const translatedTitle = this.i18n.t(item.title).toLowerCase();

      // 🔍 3. Comparar contra texto traducido
      if (
        item.type === 'link' &&
        translatedTitle &&
        translatedTitle.includes(text)
      ) {
        results.push(item);
      }
  ...
  searchRecursive(this.items);

  this.menuItems = results.slice(0, 5);
  this.SearchResultEmpty = this.menuItems.length === 0;
}

  hasPermission(menu: Menu): boolean {
    if (!menu.permission) return true;
    return this.permissionService.has(menu.permission);
  }
